    ...
    <div className='flex gap-[1rem] max-lg:flex-wrap'>
        {pricing.map((item) => (
            <div key='item.id' className='w-[19rem] max-lg:w-full h-full px-6 bg-n-8 border border-n-6 rounded-[2rem] lg:w-auto even:py-14 odd:py-8 odd:my-4 
                ...
                <Button className="w-full mb-6" href={item.price ? "/pricing":"mailto:saswatiisabella2002@gmail.com"}  white={!!item.price}>
                    {item.price ? "Get started" : "Contact us"}
                </Button>
                <ul>
                    {item.features.map((feature, index)=>(
                        <li key={index}>
                            <img src={check} alt="Check" width={24} height={24} className='flex items-start py-5 border-t border-n-6'/>
